...
int PCINFO (int argc, char *argv[])

/***********************************************************************
*
*_Title	PCINFO - Calculate information needed for photoclinometry
*
*_DESC	Calculate and report the memory requirements and the azimuth
*       of characteristics for an image to be used in pc2d, pcp, or
*       pcsi. Users will be able to use the azimuth information to
*       rotate the DEM in order to use the ISIS destriping filter
*       procedure and rotate back before going into pc2d, pcp, or
*       pcsi. The memory requirements are reported in 4-byte
*       longwords.
*
*_HIST	Feb 3 2003 Janet Barrett, Used the pc2d subroutines initpcp and
*       firstguess to write this program.
*
*_END
************************************************************************/
...
* Initialize program
******************************************************************************/
  u_std_init("PCINFO","2003-04-02",
             "Calculate information needed for photoclinometry");
  C_PPARS1->nps = 1;
...
******************************************************************************/
        ...
        data.samp = x;
        data.line = y;
        if (lev_linesamp_to_latlon(&both,&data)) goto ls_to_latlon;
        if (lev_latlon_to_linesamp(&both,&data)) goto latlon_to_ls;
	tmpdata = data;
	if (lev1_latlon_to_linesamp(&both.spi,&data)) goto latlon_to_ls;
...
******************************************************************************/
  ...
  sprintf(pcpmsg,"Azimuth of characteristics= %f",charazgrid);
  u_write_msg(3,pcpmsg);
...
******************************************************************************/
  if (C_AEPAR1->delaz != 0.0 || C_AEPAR1->aspect != 1.0) {
    ...
    border = 0.5;
    xmin = 1.0-border;
    xmax = ((FLOAT4) ins)+border;
    ymin = 1.0-border;
    ymax = ((FLOAT4) inl)+border;
  ...
  nz = (ins+1)*(inl+1);
  rot45 = ((FLOAT4) (ins+inl))/1.4142136;
  ...
  n9 = n8+nz;
  n8d = n7+nz*(2*(ins+1)+3);
  n9d = n8d+nz*(ins+2); /*Amount of memory needed to do DIRECT*/
                       /*at full resolution                  */
  if (C_AEPAR1->delaz == 0.0 && C_AEPAR1->aspect == 1.0) {
    nrot = n5+2*inl;
  } else {
  ...
  sprintf(pcpmsg,"The following memory requirements refer to 4-byte longwords:");
  u_write_msg(3,pcpmsg);
  if (n8 > nrot) {
    sprintf(pcpmsg,"Memory needed to do SOR method at full resolution: %d",n8);
  } else {
    sprintf(pcpmsg,"Memory needed to do SOR method at full resolution: %d",nrot);
  }
  u_write_msg(3,pcpmsg);
  sprintf(pcpmsg,"Memory needed to do CGM method at full resolution: %d",n9);
  u_write_msg(3,pcpmsg);
  sprintf(pcpmsg,"Memory needed to do Direct method at full resolution: %d",n9d);
  u_write_msg(3,pcpmsg);
